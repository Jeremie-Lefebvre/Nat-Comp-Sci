...
double SPATIAL_CORRELATION[S];//stimulus spatial correlation
...
////////////////////EULER SCHEME//////////////////////////////////////////////////////////////////////////				
	...
	for (int s1=0;s1<S;s1++)
	{
		...
		{
					VAR[s1] = pow(10,s1-5);
					SPATIAL_CORRELATION[s2]= 0+s2/((double)S-1);				
					//cout<<VAR[s1]<<"	"<<SPATIAL_CORRELATION[s2]<<endl;
					...
					for (int s=0;s<T-1;s++)
					{
							...
							long d=rand()%105;      
		                    long noisyseed1=(long)21*s+s*trial*187198+56*d+12*trial+1;
		                    long noisyseed2=(long)69*s*s+121*s+trial+45*d+2*1+trial; 

							eta_c[s]= sqrt(-2*log(ran2(&noisyseed1)))*cos(2*3.14159*ran2(&noisyseed2));
							
							for (int k=0;k<N;k++)
							{
								 long noisyseed3=(long)88*k+s*trial*67+56*d+12*k+d;
		                   		 long noisyseed4=(long)468*s*k+78*s+s+322*d+trial*221+k; 
		            		...
		            		for (int k=0;k<N;k++)
		            		{
			            		...
			            		OU_process[k][s+1] = OU_process[k][s]+dt*(-OU_process[k][s])+sqrt(2*VAR[s1]*dt)*(sqrt(1-SPATIAL_CORRELATION[s2])*eta_c[s]+sqrt(SPATIAL_CORRELATION[s2])*eta_i[k][s]);
...
/////////////OUTPUT//////////////////////////////////////////////////////////////////////////	
        ...
        outfile.open("ADM - Spatial Corr - Normalized myelination vs corr vs var.txt", ios::out);
         for(int s1=0;s1<S;s1++)
         {
         	for (int s2=0;s2<S;s2++)
         	{
         		  			...
         		  			outfile<<VAR[s1]<<"	"<<SPATIAL_CORRELATION[s2]<<"	"<<NORMALIZED_MYELINATION[s1][s2]<<"	"<<RATE[s1][s2]<<endl;
         }
