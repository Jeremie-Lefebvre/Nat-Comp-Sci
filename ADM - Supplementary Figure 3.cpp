...
#define T_min 1000//minimal simulation time (post transient)
...
#define T_plastic_on 40000//plasticity onset time (post transient)
#define T_plastic_off T//plasticity offset time

#define t_delay_statistics_initial 1000//initial time in which delay statistics are computed
						...
						for (int i=0;i<N;i++)
						{
							for (int j=0;j<N;j++)
							{
								...
								else
								{
									...
									c[i][j]=c[i][j]+dt*((c_min-c[i][j])*alpha_retraction+2*X_e[j][t-1]*(l[i][j]/(c[i][j]))*alpha_formation);
									slope[i][j] = 	((c_min-c[i][j])*alpha_retraction+2*X_e[j][t-1]*(l[i][j]/(c[i][j]))*alpha_formation);
