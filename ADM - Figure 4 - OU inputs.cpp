...
////////////////////EULER SCHEME//////////////////////////////////////////////////////////////////////////				
	...
	for (int s1=0;s1<S;s1++)
	{
		...
		{
					VAR[s1] = pow(10,s1-5);
					CORRELATION_TIME[s2]= pow(10,s2-1);				
					//cout<<VAR[s1]<<"	"<<CORRELATION_TIME[s2]<<endl;
					...
					for (int s=0;s<T-1;s++)
					{
							...
							long d=rand()%105;      
		                    long noisyseed1=(long)21*s+s*trial*187198+56*d+12*1+trial;
		                    long noisyseed2=(long)69*s*s+11*s+s+45*d+2*trial+1; 
		            		eta[s]= sqrt(-2*log(ran2(&noisyseed1)))*cos(2*3.14159*ran2(&noisyseed2));
		            		
		            		OU_process[s+1] = OU_process[s]+dt*(-(1/CORRELATION_TIME[s2])*OU_process[s])+sqrt(2*VAR[s1]*dt)*eta[s];
					}
		        	...
		        	for (int s=0;s<T_window-1;s++)
					{
		        		...
		        		{
		                            long d=rand()%105;      
		                            long noisyseed1=(long)21*s+n*n*187198+56*d+12*trial+1;
						...
						//	u_theo[T_window-1] = u_theo[T_window-1]+dt*alpha_u*(-1*u_theo[T_window-1]+sum_theo+I);
							u_e[i][T_window-1]=u_e[i][T_window-1]+dt*(-1*u_e[i][T_window-1]+sum_ee+I);
										  ...
										  long seed_q =rand()%101;  
						   				  long seed_e = (long) 12+i*i+s+15+s*seed_q+56*i+trial+99*s+2;              
...
/////////////OUTPUT//////////////////////////////////////////////////////////////////////////	
        ...
        outfile.open("ADM - OU process - Normalized myelination vs temporal corr vs var.txt", ios::out);
         for(int s1=0;s1<S;s1++)
         {
         	for (int s2=0;s2<S;s2++)
         	{
         		  			...
         		  			outfile<<VAR[s1]<<"	"<<CORRELATION_TIME[s2]<<"	"<<NORMALIZED_MYELINATION[s1][s2]<<"	"<<RATE[s1][s2]<<endl;
         }
                         
         }  
        outfile.close(); 

   		 outfile.open("ADM - OU process.txt", ios::out);
         for(int t=0;t<T;t++)
         {
         	outfile<<t<<"	"<<OU_process[t]<<endl;
